...
# Upstream ref: GetFinOpsHighImpactQueriesAsync (DatabaseService.FinOps.Queries.cs)
_SQL = f"""
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;

DECLARE @cutoff datetime2(7) = {tz_from()};
DECLARE @cutoff_end datetime2(7) = {tz_to()};
...
WITH
    ...
    (
        SELECT
            ...
            executions_share =
                CONVERT(decimal(5,1),
                    100.0 * a.total_executions /
                    NULLIF(SUM(CONVERT(float, a.total_executions)) OVER (), 0)),
            cpu_share =
                CONVERT(decimal(5,1),
