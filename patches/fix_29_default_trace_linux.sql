/*
Upstream bug fix: collect.default_trace_collector fails on Linux SQL Server.

Root cause: sys.fn_trace_gettable is called with an expression that strips the
_N rollover suffix from sys.traces.path (e.g. log_1.trc -> log.trc).
On Linux, SQL Server names the initial trace file without a suffix (log.trc),
so CHARINDEX('_', REVERSE(path)) returns 0, causing LEFT to take the full path
and RIGHT to append .trc again, producing log.trc.trc.

Fix: fall back to st.path when CHARINDEX returns 0

Upstream: https://github.com/erikdarlingdata/PerformanceMonitor install/29_collect_default_trace.sql
*/
    ...
    */
    ...
    BEGIN TRY
        ...
        */
        ...
        (
            /*
            Fix for Linux SQL Server: when sys.traces.path has no _N rollover suffix
            (e.g. /var/opt/mssql/log/log.trc), CHARINDEX returns 0 and the original
            expression produces log.trc.trc. Fall back to st.path when no underscore found.
            */
            CASE
...
PRINT 'Patch applied: fix_29_default_trace_linux - fn_trace_gettable path fix for Linux SQL Server';
GO
